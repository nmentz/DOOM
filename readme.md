...
Build, run, and debugging is all done with a python script named ```script```.

```bash
(shell)$ ./script
flags
    b, build, - builds DOOOM
    d, debug, - puts user directly into gdb debugging session
    r, run, - launches application
...
---
 - Documentation is in ```./docs/```. All docs can be rendered in your browser locally.
 - a feature
 - another feature
